...
//	std::vector<WCSimRootTrack> truetracks;	// nice but depends on WCSim classes
	...
	std::vector<std::pair<TVector3,TVector3>> GetTrueTrackVertices(){return truetrackvertices;}
	std::vector<Int_t> GetTrueTrackPdgs(){return truetrackpdgs;}
...
//		aspectrumv.assign(temp.begin(), temp.end());
	...
	cMRDSubEvent() : mrdsubevent_id(-1), wcsimfile(""), run_id(-1), event_id(-1), trigger(-1), digi_ids(), pmts_hit(), digi_qs(), digi_ts(), digi_numphots(), digi_phot_ts(), digi_phot_parents(), layers_hit(), eDepsInLayers(), tracksthissubevent(), truetrackvertices(), truetrackpdgs()  {};
